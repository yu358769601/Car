...
class CheckUpdateTask extends AsyncTask<Void, Void, String> {
    ...
    CheckUpdateTask(Context context, int type, boolean showProgressDialog,  CallBack callBack) {
    ...
    protected void onPreExecute() {
        Log.i("CheckUpdateTask", "onPreExecute:刚启动的时候使用的方法 ");
    ...
    protected void onPostExecute(String result) {
        Log.i("CheckUpdateTask", "onPostExecute: 获取网络数据完成");
    ...
    private void parseJson(String result) {
        try {
            Log.i("CheckUpdateTask", "parseJson: 解析网络过来的json 分析是否需要更新");
            ...
            //如果是强制更新版本
            int apkforceUpData = obj.getInt(Constants.APK_VERSION_FORCEUPDATA);
            ...
            if (apkCode > versionCode) {
                if (mType == Constants.TYPE_NOTIFICATION) {
                    ...
                    showDialog(mContext, updateMessage, apkforceUpData,apkUrl,callBack);
                }
    ...
    protected String doInBackground(Void... args) {
        Log.i("CheckUpdateTask", "doInBackground: 获取网络数据");
